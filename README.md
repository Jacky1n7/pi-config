...
[Pi](https://pi.dev) 是一个开源的终端 AI 编码代理（由 `@earendil-works/pi-coding-agent` 提供）。你给它一个任务，它在你本地的项目里读写文件、跑命令、调用工具来完成任务。它的核心特点：

- **插件化**：能力通过 npm 包扩展（叫 extension / skill / prompt / theme），用 `pi install` 安装
- **多模型**：支持 OpenAI / Anthropic / 本地模型等多种 provider，可以随意切换
- **工具驱动**：模型通过一组工具（bash / read / edit / web_search / mcp …）操作你的环境
- **Skill 机制**：把「怎么做某类任务」沉淀成可复用的过程知识，跨会话存活
...
跨会话记忆。让 Pi 记住你之前告诉过它的事（你的偏好、项目约定、之前踩过的坑），下次开新会话还能用上。记忆分 user / memory / project / failure 四类，可搜索。
...
MCP（Model Context Protocol）适配器。让 Pi 能连接任何 MCP server，把它的工具接进来。本仓库的 2 个 MCP server 就是通过它生效的。
...
状态栏增强。在底部显示模型、上下文占用、git 状态等信息，一眼看清当前状态。
...
精选主题集 + `adapt-ghostty-theme-to-pi` skill：把 Ghostty 终端主题迁移成 Pi 主题。
...
> 其中 `chrome-devtools` 是我放在 `~/.agents/skills/` 的自定义全局 skill，不在任何 npm 包里，需要自己创建（参考 Pi 的 skill 文档）。
